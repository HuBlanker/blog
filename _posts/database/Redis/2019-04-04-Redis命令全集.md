...
总的来说,Redis是一个基于内存的高性能的键值型数据库,也就是常说的NoSQL,可以用来作为数据库或者缓存.并且支持多种数据结构,包括字符串，散列，列表，集合，带有范围查询的排序集，位图，超级日志(直译,具体翻译不确定)，具有半径查询的地理空间索引和流.
...
- **SDIFF**: 返回第一和集合和其他集合不同的元素.`SDIFF key1 key2 key3`.
例如:
```
key1 = {a,b,c,d}
key2 = {c}
...
- **SDIFFSTORE**: 和上一个命令差不多,只不过会把结果存在第一个set中,覆盖存储.`SDIFFSTORE key1 key2 key3`.O(n)的时间复杂度.
...
- **HGET**: 获取某个key的某个列的值.`HGet key field`.
